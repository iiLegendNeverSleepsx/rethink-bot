...
module.exports.run = async (bot, client, response, args) => { 
  ...
  if (response.mentions.members.first() || response.guild.members.get(args[0])) {
    ...
    if (user.member.hasPermission("KICK_MEMBERS")) return response.reply("that user can not be kicked!");
    if (user.managable) {
      args.shift();
      user.kick(args.join(" "))
    }
    ...
    } else if (typeof thing === 'undefined') { response.reply("give a user to kick!");
    } else if (thing === null) { response.reply("no users were found with your term.")
    } else {
      let user = thing;
      if (user.member.hasPermission("KICK_MEMBERS")) return response.reply("that user can not be kicked!");
      if (user.managable) {
        args.shift()
        user.kick(args.join(" "));
      } else {response.reply("I can not kick that user! Do I have the permission to kick members? Is the user's highest role not higher than mine?")}
    }
  } else {response.reply("Can't find that user!")}
} 
